...
**All coaches (v1.11, normative).** The coach selector's first entry is **All coaches**, for a week or date on which nobody meets — a public holiday, a campus closure. It is offered whether or not any availability has been uploaded. An all-coaches block is stored once, against a sentinel coach, and is **expanded to every coach the run knows about at the moment the schedule is built**, so it also covers coaches whose availability arrives after the block was added; §11.3 then treats each expanded (coach, week/date) exactly as a block added for that coach by name, and every §11.3 and §9 rule applies unchanged. In the panel it is listed first, under "All coaches", and removing it lifts the block for every coach at once. With one coach selected, a week blocked for all coaches shows in that coach's ribbon as blocked and visibly inherited; clicking it does not unblock it — that is done from the All coaches selection, so one coach's row can never quietly change every other coach's schedule.
